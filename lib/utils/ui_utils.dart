...
class UiUtils {
  ...
  static Widget placeholderWidget(BuildContext context,
      double? width,
      double? height,) {
    return Container(
      width: width,
  ...
  static void setDefaultLocationValue({required bool isCurrent,
  ...
  static String convertToAgo({
    ...
    if (diff.inDays >= 365) {
      ...
      return "${years.toStringAsFixed(0)} ${'years'.translate(context)} ${'ago'
          .translate(context)}";
    } else if (diff.inDays >= 30) {
      int months = calculateMonthsDifference(input, DateTime.now());
      return "${months.toStringAsFixed(0)} ${'months'.translate(
          context)} ${'ago'.translate(context)}";
    } else if (diff.inDays >= 1 || (isNegative && diff.inDays < 1)) {
      return "${diff.inDays} ${'days'.translate(context)} ${'ago'.translate(
          context)}";
    } else if (diff.inHours >= 1 || (isNegative && diff.inMinutes < 1)) {
      return "${diff.inHours} ${'hours'.translate(context)} ${'ago'.translate(
          context)}";
    } else if (diff.inMinutes >= 1 || (isNegative && diff.inMinutes < 1)) {
      return "${diff.inMinutes} ${'minutes'.translate(context)} ${'ago'
          .translate(context)}";
    } else if (diff.inSeconds >= 1) {
      return "${diff.inSeconds} ${'seconds'.translate(context)} ${'ago'
          .translate(context)}";
    } else {
